...
# Tokenizer for text (load once only)
with open(os.path.join(BASE_DIR, 'models/tokenizer_config.json'), 'r') as f:
    tokenizer = tokenizer_from_json(f.read())
...
def predict_image(img_path):
    model = load_model_from_json(
        os.path.join(BASE_DIR, 'models/emotion_modelIV.json'),
        os.path.join(BASE_DIR, 'models/modelIV.weights.h5')
    )
...
def predict_realtime_video():
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            label = int(np.argmax(preds))
            smooth.append(label)
            display_label = IMG_LABELS[max(set(smooth), key=smooth.count)]
            cv2.rectangle(frame, (x, y-50), (x+w, y+h+10), (255, 0, 0), 2)
            cv2.putText(frame, display_label, (x+20, y-60), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2, cv2.LINE_AA)
        cv2.imshow("Real-time Emotion", cv2.resize(frame, (1280, 720)))
        if cv2.waitKey(1) & 0xFF == ord('q'):
...
def extract_features(file_path, max_pad_len=MAX_LEN):
    y, sr = librosa.load(file_path, sr=SAMPLE_RATE)
    mfcc = librosa.feature.mfcc(y=y, sr=sr, n_mfcc=N_MFCC)
    pad = max_pad_len - mfcc.shape[1]
    mfcc = np.pad(mfcc, ((0, 0), (0, max(0, pad))), mode='constant')[:, :max_pad_len]
    return mfcc

def predict_audio(audio_path):
    model = load_model_from_json(
        os.path.join(BASE_DIR, 'models/audio_emotion_model.json'),
        os.path.join(BASE_DIR, 'models/audio_model.weights.h5')
    )

    mfcc = extract_features(audio_path)
    mfcc = mfcc[np.newaxis, ..., np.newaxis]
    pred = model.predict(mfcc, verbose=0)[0]
    emotion = AUDIO_LABELS[np.argmax(pred)]
...
def predict_text(text):
    model = load_model_from_json(
        os.path.join(BASE_DIR, 'models/text_model.json'),
        os.path.join(BASE_DIR, 'models/text_model.weights.h5')
    )

    seq = tokenizer.texts_to_sequences([text])
    pad = pad_sequences(seq, maxlen=MAX_SEQ_LEN)
    pred = model.predict(pad, verbose=0)[0]
    emotion = TEXT_LABELS[np.argmax(pred)]
